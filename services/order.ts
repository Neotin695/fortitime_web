...
export const orderService = {
  create: async (data: OrderCreateBody) =>
    fetcher.post<DefaultResponse<{ id: number }>>("v1/dashboard/user/orders", {
      body: data,
    }),
  ...
  get: (id?: number | null) =>
    fetcher<DefaultResponse<OrderFull>>(`v1/dashboard/user/orders/${id}`),
  cancel: (id?: number) =>
  ...
  downloadInvoice: (id?: number) =>
    fetch(`${BASE_URL}v1/dashboard/user/export/order/${id}/pdf`, {
      headers: {
