...
const shopSchema = yup.object({
  ...
  phone: yup.string().required("Phone number is required").min(12, "invalid.format"),
  title: yup.object({
...
const BecomeSellerForm = () => {
  ...
  const handleCreateStore = (data: CreateShopBody) => {
    const { logo_image, bg_image, delivery_time_type, location, ...dataForServer } = data;
    createStore(
      {
        ...dataForServer,
        images: [bg_image, logo_image],
