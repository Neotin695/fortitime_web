...
const schema = yup
  .object({
    ...
    phone: yup.string().required().min(12, "invalid.format"),
    zipcode: yup.string().required(),
    street_house_number: yup.string().required(),
...
const AddressForm = ({
  ...
  const handleMapClick = (e: google.maps.MapMouseEvent) => {
    ...
    searchAddress(
      ...
      {
        onSuccess: (res) => {
          setValue("location.address", res?.results[0]?.formatted_address);
        },
      }
    );
  };
        ...
        <div className="col-span-4 flex items-center gap-4">
          {!!onCancel && (
            <Button
              ...
              onClick={onCancel}
              type="button"
              disabled={isPageChanging || isButtonLoading}
            >
              {t("cancel")}
            </Button>
          )}
          <Button type="submit" loading={isPageChanging || isButtonLoading} fullWidth color="black">
            {t("next")}
          </Button>
        </div>
      </div>
