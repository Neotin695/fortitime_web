...
export const ProductStickyInfo = ({ data, selectedStock }: ProductStickyInfoProps) => {
  ...
  const { isLiked, handleLikeDisLike } = useLike(data?.id);
  ...
  return (
    <div
      className={clsx(
        "md:sticky block md:top-0 md:bottom-auto w-full fixed bottom-0 rounded-t-md md:rounded-t-none",
            ...
            {cartQuantity && cartQuantity > 0 ? (
              <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-normal">
                ...
                <div className="flex items-center gap-4">
                  <IconButton
                    disabled={data && cartQuantity <= data?.min_qty}
