...
const ReviewSummary = ({ type, typeId }: ReviewSummaryProps) => {
  const { t } = useTranslation();
  const { data } = useQuery(["groupRating", type, typeId], () =>
    reviewService.getRating(type, typeId)
  );
  return (
    <div className="grid grid-cols-4 gap-4">
      <div className="flex flex-col gap-4">
        <div className="border border-gray-border dark:border-gray-bold rounded-2xl flex items-center justify-center flex-col flex-1">
          ...
          <span className="text-xs font-medium">{t("total.rating")}</span>
        </div>
        <div className="border border-gray-border dark:border-gray-bold  rounded-2xl flex items-center justify-center py-7">
          <span className="text-yellow">
            <StarSmileIcon />
          </span>
          <span className="text-xs font-medium">
            {data?.count || 0} {t("reviews")}
          </span>
        </div>
      </div>
